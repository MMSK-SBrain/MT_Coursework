#!/usr/bin/env python3
"""
Convert REVISED YAML learning outcomes to Excel template
Reads: 03-learning-outcomes-REVISED.yaml
Writes: 03-template-REVISED.xlsx
"""
...
# Read YAML file
print("Reading REVISED YAML file...")
with open('03-learning-outcomes-REVISED.yaml', 'r') as f:
...
course_info = [
    ...
    ["Duration (Days)", str(course['duration_days'])],
    ["Description", course['description']],
    ["Institution", course['institution']],
    ["Credits", str(course['credits'])],
    ["Total Sessions", str(course['total_sessions'])],
    ["Session Duration", course['session_duration']],
    ["Pedagogical Approach", course['pedagogical_approach']],
]
...
# Save workbook
output_file = "03-template-REVISED.xlsx"
wb.save(output_file)
print(f"✓ Excel template created: {output_file}")
print(f"✓ Total rows in Outcomes_And_Structure: {row_num - 1}")
print(f"✓ Modules: {len(data['modules'])}")
print(f"✓ Total sessions: {sum(len(m['lessons']) for m in data['modules'])}")
print(f"✓ Total session outcomes: {data['summary']['total_session_outcomes']}")
print(f"\n✓ REVISED structure with physics/math foundation first!")
print(f"  - Sessions 1-3: Vehicle Motion Fundamentals (NEW)")
print(f"  - Sessions 4-8: Powertrain Systems")
print(f"  - Sessions 9-12: Chassis Dynamics & Control")
print(f"  - Sessions 13-20: Electrical, Architecture, Safety & Future")
